...
/**
 * Ambient Three.js backdrop for the intro sequence — a soft, low-poly
 * rolling terrain under a gradient dawn sky, with a few drifting motes of
 * light and a gentle mouse-parallax. A from-scratch homage to the visual
 * idea behind threeui.com's "Landscape" scene, tuned down to sit quietly
 * behind the terminal + greeting rather than compete with them.
 */
export function LandscapeScene({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const canParallax = window.matchMedia('(hover: hover)').matches && !reducedMotion

    let renderer: THREE.WebGLRenderer
    try {
      renderer = new THREE.WebGLRenderer({ canvas, antialias: true, alpha: false, powerPreference: 'low-power' })
    } catch {
      return // No WebGL: canvas stays invisible, page background shows through.
    }
    ...
    if (canParallax) window.addEventListener('mousemove', onMouseMove)
    ...
    function loop() {
      renderFrame()
      rafId = reducedMotion ? null : requestAnimationFrame(loop)
    }
    ...
    if (!reducedMotion) {
      ...
      onVisibilityChange = () => {
        if (document.hidden) {
          ...
          lastTime = performance.now() // drop the paused-time delta
          rafId = requestAnimationFrame(loop)
        }
      }
    ...
    return () => {
      ...
      if (canParallax) window.removeEventListener('mousemove', onMouseMove)
  ...
  return <canvas ref={canvasRef} className={`intro-landscape ${className ?? ''}`.trim()} aria-hidden="true" />
}
