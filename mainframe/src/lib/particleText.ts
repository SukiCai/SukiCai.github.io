 ...
 * Shared by the hero name and the intro greeting/signature.
 */
...
export function createParticleText(
  ...
  const textElQuery = heading.querySelector<HTMLElement>('.particle-name-text')
  const ctx2d = canvas.getContext('2d')
  if (!textElQuery || !ctx2d) return { play() {}, retarget() {}, destroy() {} }
  const ctx = ctx2d
  const textEl = textElQuery

  const dpr = Math.min(window.devicePixelRatio || 1, 2)

  const INK = options.colors?.ink ?? [20, 20, 20]
  const INK_SOFT = options.colors?.inkSoft ?? [85, 83, 77]
  const ACCENT = options.colors?.accent ?? [47, 95, 219]
  ...
  function sampleTargets(): Target[] {
    ...
    probe.font = font
    if ('letterSpacing' in probe) (probe as CanvasRenderingContext2D).letterSpacing = cs.letterSpacing
  ...
  function pickColor(target: Target): { rgb: [number, number, number]; sparkle: boolean } {
  ...
  function makeParticle(target: Target, scattered: boolean): Particle {
  ...
  function init(scattered: boolean) {
  ...
  const onResize = () => {
    ...
    resizeTimer = window.setTimeout(() => init(false), 200)
  }
